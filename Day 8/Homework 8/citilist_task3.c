...
bst *insertBST(bst *root, city *newCity) {
    ...
    if(newCity->countryFips == root->cityData->countryFips) {
        return root;
    }
    if(newCity->countryFips < root->cityData->countryFips) {
...
int main() {
    ...
    FILE *inFile = fopen("../../Resources/uscities.csv", "r");
        if (inFile == NULL) {
            ...
            // Do the binary search for FIPS 36081
            int low = 0;
            int high = index - 1;
            int target = 36081;
            ...
            while(low<=high) { // Start with the mid of the array, because the array is already sorted
                ...
                if(currentFIPS == target) {
                    printf("By FIPS code, %s is index %d\n", cityArr[mid]->name, mid);
                    break;
                }else if(target < currentFIPS) {
                    high = mid - 1;
                }else{
                    low = mid + 1;
                }
