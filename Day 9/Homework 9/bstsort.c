...
Node *insert(Node *root, int value) {
    // Create a new node if it's empty
    if(root == NULL) {
    ...
    // Insert smaller values to the left
    if(value < root->value) {
        root->left = insert(root->left, value);
    }
    // Insert the larger or equal value to the right
    else{
...
// Perform in-order traversal and copy the sorted value back to the array
...
void sortarray (int a[], int n) {
     ...
     * 3. Copy sorted values back into array
     * 4. Free BST memory
     */

    Node *root = NULL; // Initialize the tree
