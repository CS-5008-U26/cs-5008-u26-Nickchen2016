...
void sortarray (int a[], int n) {
    ...
    /* Selection Sort
     * Select the smallest item from the unsorted partition and move it to the sorted partition.
     */

    for(int i=0; i<(n-1); i++) {
        int min = i; // Placeholder for the index of the smallest number
        for(int j=(i+1); j<n; j++) { // 
            if(a[j]<a[min]){
                min = j;
            }
        }
        // Swap smallest value into position i
        if(min != i){
            int temp = a[i];
            a[i] = a[min];
            a[min] = temp;
