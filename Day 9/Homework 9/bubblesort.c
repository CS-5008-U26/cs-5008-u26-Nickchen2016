...
void sortarray (int a[], int n) {
     ...
     * Stops early if a complete pass makes no exchanges.
     */

    for(int i=0; i<(n-1); i++) {
        int exchanges = 0; // Numbers need to swap in this pass
        for(int j=0; j<(n-1-i); j++) { // 'j<(n-1-i)' reset the array length after each pass. Because the rightmost number has already been sorted
            if(a[j]>a[j+1]){
                int temp = a[j];
                a[j] = a[j+1];
                a[j+1] = temp;
        ...
        if(exchanges == 0){ // If there is no swaps, the array is already sorted
            break;
        }
    }
