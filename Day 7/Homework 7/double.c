...
void insertFront (dNode **head, dNode *newNode) { // single astrisk is the actual node point to, double astrisk is a pointer that you can modify, so you can change the head of the list
...
void insertEnd (dNode **head, dNode *newNode) {
    if(head == NULL || newNode == NULL) {
        return;
    }
    newNode->next = NULL;
...
// Get the nth node
dNode *getNode(dNode *head, int n) {
    if(n<0) {
        return NULL;
    }
    int i = 0;
...
int main() {
    FILE *inFile = fopen("../../Resources/uscities.csv", "r");
        if (inFile == NULL) {
            ...
            dNode *head = NULL; // Create a link list and set the head to NULL
            for (int lineCount = 0; lineCount <20; lineCount++) {
                ...
                if(!fgets(inputLine, sizeof(inputLine), inFile)) {
                    ...
                    dNode *newNode = createNode(newCity); // Create a new node with the city data
            ...
            while(isGoing) {
                ...
                if(strcmp(command, "size") == 0) {
                    ...
                    dNode *n = getNode(head, (secondaryCommand-1));
                    deleteNode(&head, n);
                    ...
                    scanf("%d", &secondaryCommand);
                    dNode *node = getNode(head, (secondaryCommand-1));
                    if(node == NULL) {
